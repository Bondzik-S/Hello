...
function show(dataBase) {
  document.write(
    `<header><span>${dataCurrency.date}</span><span class="header">Exchange rate</span></header><table><tr><th colspan="2" class="center">Country</th><th class="center">Currency</th><th class="left">We buy at</th><th class="left">We sell at</th></tr>`
  );
  dataBase.exchangeRate.forEach((element) =>
    document.write(
      `<tr><td class="center"><div class="image" style="background-position: ${
        element.flag
      };"></div></td><td class="left">${
        element.country
      }</td><td class="center">${
        element.currency
      }</td><td class="red">${element.purchaseRateNB.toFixed(
        2
      )}</td><td class="red">${element.saleRateNB.toFixed(2)}</td></tr>`
    )
  );
